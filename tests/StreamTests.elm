...
import Stream exposing (Stream, cons, empty, toList, map, filter, 
    take, append, concatMap, foldl, foldr, zip, scan, fromList)
...
suite =
    ...
    , describe "given a stream of streams of numbers"
        [ test "when flattening them to a single stream of numbers" <|
            \_ ->
                let
                    first = (cons 100 (\() -> cons 200 (\() -> empty)))
                    second = (cons 1000 (\() -> cons 2000 (\() -> empty)))
                    third = (cons 10000 (\() -> cons 20000 (\() -> empty)))
                    streamOfStreams =
